...
PImage rawImg, grayImg, maskedImg, backImg;
...
void setup()
{
 ...
 //       // Comment the following line if you use Processing 1.5
  cam.start();
	...
	rawImg = new PImage(80,60); 
	theBlobDetection = new BlobDetection(rawImg.width, rawImg.height);
	theBlobDetection.setPosDiscrimination(true);
	theBlobDetection.setThreshold(0.3f); // will detect bright areas whose luminosity > 0.2f;

  opencv = new OpenCV(this, rawImg.width, rawImg.height);
...
void draw()
{
	if (newFrame)
	{
		...
		rawImg.copy(cam, 0, 0, cam.width, cam.height, 
				0, 0, rawImg.width, rawImg.height);
    
    opencv.loadImage(rawImg);
    if(backgroundCaptured){
      opencv.diff(backImg);
      grayImg = opencv.getSnapshot();
    }else{
      grayImg = rawImg;
    }
    ...
    //grayImg.filter(GRAY);
		...
		fastblur(grayImg, 2);
		theBlobDetection.computeBlobs(grayImg.pixels);
