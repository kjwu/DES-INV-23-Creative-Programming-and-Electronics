  ...
  void display(){
    ...
    pushMatrix();
      ...
      pushStyle();
        ...
        //noFill();
        stroke(age*15+initialColor,60,60); 
                fill(age*15+initialColor,60,60,2);
        ...
        if(showStones){
          noStroke();
          fill(age*4+initialColor,60,100);
          for(int i = 0; i < 360; i +=15){
            float shift = random(-5,5);
            ellipse(size*cos(radians(i+shift))/2,size*sin(radians(i+shift))/2, 3,3);
          }
        }
